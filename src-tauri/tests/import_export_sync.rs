...
fn sync_codex_provider_writes_auth_and_config() {
    ...
    let auth_path = cc_inyx_lib::get_codex_auth_path();
    let config_path = cc_inyx_lib::get_codex_config_path();
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = cc_inyx_lib::get_codex_config_path();
    ...
    cc_inyx_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = cc_inyx_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        cc_inyx_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        cc_inyx_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        cc_inyx_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !cc_inyx_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !cc_inyx_lib::get_codex_config_path().exists(),
...
"#;
    ...
    cc_inyx_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        cc_inyx_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = cc_inyx_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        cc_inyx_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        cc_inyx_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = cc_inyx_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    cc_inyx_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = cc_inyx_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = cc_inyx_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".cc-inyx").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".cc-inyx");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.cc-inyx/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("cc-inyx-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_cc_inyx_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
...
#[test]
fn import_sql_accepts_cc_inyx_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("cc-inyx-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
